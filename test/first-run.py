def get_template_columns(ref_gwas, target_gwas, right_on, left_on):
    ...
    merged = ref_gwas.merge(target_gwas, right_on = right_on, left_on = left_on, how = 'right')

    original_SNP_name = [el for el in target_gwas.columns if 'SNP' in el][0]
    merged_dedup = merged.drop_duplicates(subset = f'{original_SNP_name}_y', keep = 'first')
    ...
    try:
        template = res.loc[:, [right_on, 'SNP_y', 'ALLELE1_y', 'ALLELE0_y', 'A1FREQ_y', 'BETA_y']]
    except:
        template = res.loc[:, ['SNP_y', 'SNP_y', 'ALLELE1_y', 'ALLELE0_y', 'A1FREQ_y', 'BETA_y']]
    template.columns = ['rsid_harmonized'] + [el + '_harmonized' for el in template.columns.str.replace('_y', '')][1:]
    return template